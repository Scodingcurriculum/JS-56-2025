...
function showDetails(eventKey) {
  ...
  window.onclick = function(event) {
    ...
    if (lat >= 20 && lat <= 30 && lon >= 70 && lon <= 90) return "Asia/Kolkata"; // India
    if (lat >= 35 && lat <= 45 && lon >= -120 && lon <= -70) return "America/New_York"; // USA East
    if (lat >= 35 && lat <= 40 && lon >= 135 && lon <= 140) return "Asia/Tokyo"; // Japan
    if (lat >= 50 && lat <= 55 && lon >= 0 && lon <= 10) return "Europe/London"; // UK
    if (lat >= -35 && lat <= -30 && lon >= 140 && lon <= 150) return "Australia/Sydney"; // Australia
...
function calculateAge() {
  ...
  function addStar() {
    ...
    star.style.left = Math.random() * (area.clientWidth - 20) + 'px';
    star.style.top = Math.random() * (area.clientHeight - 20) + 'px';

    // Dragging behavior
    star.addEventListener('mousedown', function (e) {
      const offsetX = e.clientX - star.offsetLeft;
      const offsetY = e.clientY - star.offsetTop;

      function moveStar(ev) {
        star.style.left = (ev.clientX - offsetX) + 'px';
        star.style.top = (ev.clientY - offsetY) + 'px';
      }
